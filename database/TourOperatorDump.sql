...
INSERT INTO `service` VALUES (1,'Такси','Taxi','Такси от аэропорта до отеля','Taxi from the airport to the hotel',20000),(2,'Завтрак','Breakfast','Завтрак в номер','Breakfast room service',50000),(3,'Няня для ребенка','Baby sitter','Специально обученный человек будет развлекать вашего ребенка пока вы отдыхаете','A specially trained person will entertain your child while you are resting',120000),(4,'Аренда транспорта','Transport rental','Вам предоставят транспорт на аренду, на весь срок вашего отдыха','You will be provided with transport for rent, for the entire duration of your holiday',125000);
...
CREATE TABLE `service_contract` (
  ...
  KEY `service_contract_fk_1_idx` (`id_contract`),
  KEY `service_contract_fk_2_idx` (`id_service`),
  CONSTRAINT `service_contract_fk_1` FOREIGN KEY (`id_contract`) REFERENCES `contract` (`id`),
  CONSTRAINT `service_contract_fk_2` FOREIGN KEY (`id_service`) REFERENCES `service` (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_0900_ai_ci;
/*!40101 SET character_set_client = @saved_cs_client */;
...
INSERT INTO `service_contract` VALUES (23,3),(26,2);
